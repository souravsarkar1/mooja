...
const FriendRequestToMe = () => {
  const [friendRequests, setFriendRequests] = useState<FriendRequest[]>([]);
  const [loading, setLoading] = useState(true);
const handleAcceptFriendRquest = async(id : string)=>{
    try {
        const data = await api.post("/users/accept-request", {requesterId : id});
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.post("/users/get-friend-request");
...
<div>
    <Button onClick={()=> navigate("/profile/friend-requests-by-me")}>Friend Request Sented</Button>
</div>
      {loading ? (
